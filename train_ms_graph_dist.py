...
ms.context.set_context(device_id=device_id) # set device_id
...
def mktrainval(args, logger):
    ...
    if args.dataset == "cifar10":
        ...
        train_set = ds.Cifar10Dataset(args.datadir, "train", shuffle=True, num_shards=rank_size, shard_id=rank_id)
        valid_set = ds.Cifar10Dataset(args.datadir, "test", shuffle=True, num_samples=1000)
        test_set = ds.Cifar10Dataset(args.datadir, "test")
    else:
    ...
    train_set = train_set.shuffle(num_train)
    ...
    test_set = test_set.map(operations=val_transform, input_columns=["image"])
    test_set = test_set.map(operations=type_cast_op, input_columns=["label"])
    test_set = test_set.batch(batch_size=micro_batch_size)

    return train_set, valid_set, test_set, num_train, num_val
...
def main(args):
    ...
    train_set, valid_set, test_set, num_train, num_val = mktrainval(args, logger)
    logger.info(f"dataset batch size: {train_set.get_batch_size()}")

    #======dataparallel=======
    logger.info("Moving model onto all NPUs")
    ### model = torch.nn.DataParallel(model)
    ms.context.set_auto_parallel_context(parallel_mode=ParallelMode.DATA_PARALLEL, gradients_mean=True)
    ...
    param_dict = ms.load_checkpoint(f"../{args.model}.ckpt")
    ...
    for x, y in recycle(train_set):
        ...
        # if device_id == 0:
        #     wandb.log({"loss": float(loss.asnumpy())})
        step_cnt += 1
    ...
    logger.info("Starting evaluating on the whole test set!")
    with tqdm(total=test_set.get_dataset_size()) as prog:
        for (_x, _y) in tqdm(test_set):
            logits, y_truth = eval_net(_x, _y)
            metric.update(logits, y_truth)
            prog.update(1)
    accuracy, temp1, temp2 = metric.eval()
