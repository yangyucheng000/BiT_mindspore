...
def mixup_data(x, y, l):
    ...
    # indices = ops.Randperm(max_length=x.shape[0])(ms.Tensor([x.shape[0]],ms.int32))
...
class train_step(nn.TrainOneStepCell):
    ...
    def __init__(self, network, loss_func, optimizer, mixup, args, logger):
       super().__init__(network, optimizer)
       self.grad = ops.GradOperation(get_by_list=True)
       self.mixup = mixup
       self.mixup_l = np.random.beta(mixup, mixup) if mixup > 0 else 1
       self.loss_func = loss_func
       self.step, self.accum_step = 0, 0
       self.accum_grads = []
       self.args = args
       self.logger = logger
       self.net_loss_mixup = NetWithLossMixup(network, loss_func)
       self.net_loss = nn.WithLossCell(network, loss_func)
    ...
    def construct(self, data, label):
        ...
        if self.mixup > 0:
            x, y_a, y_b = mixup_data(data, label, self.mixup_l)
            c = self.net_loss_mixup(x, y_a, y_b, self.mixup_l)
            # logits = self.network(x)
            # c = mixup_criterion(self.loss_func, logits, y_a, y_b, self.mixup_l)
            # c = self.net_loss_mixup(data, label, self.mixup_l)
            grads = self.grad(self.net_loss_mixup, weights)(x, y_a, y_b, self.mixup_l)
            # for testing
            # self.logger.info(f"====grads before reducer: {grads[0].asnumpy()[0]}====")
            grads = self.grad_reducer(grads)
            # self.logger.info(f"====grads after reducer: {grads[0].asnumpy()[0]}====")
            if len(self.accum_grads) == 0:
                for i in grads:
                    self.accum_grads.append(ops.ZerosLike()(i))
            self.accum_grads = [self.accum_grads[i] + grads[i] / batch_split for i in range(len(grads))]
            self.accum_step += 1
        else:
            # logits = self.network(data)
            # c = self.loss_func(logits, label)
        ...
        # if self.mixup > 0:
        #     # function to compute grads, return a tuple
        #     grads = self.grad(lambda _x, _y_a, _y_b: mixup_criterion(
        #         self.loss_func, self.network(_x), _y_a, _y_b, self.mixup_l) / batch_split, weights)(x, y_a, y_b)
            
        #     self.accum_grads = (self.accum_grads[i] + grads[i] for i in range(len(grads)))
        #     self.accum_grads += 1
        # else:
        #     grads = self.grad(lambda _x, _y: self.loss_func(self.network(_x), _y) / batch_split, weights)(data, label)
        #     self.accum_grads = (self.accum_grads[i] + grads[i] for i in range(len(grads)))
        #     self.accum_grads += 1
        
        accstep = f" ({self.accum_step}/{self.args.batch_split})" if self.args.batch_split > 1 else ""
        # self.logger.info(f"[step {self.step}{accstep}]: loss={c_num:.5f} (lr={float(self.optimizer.get_lr().asnumpy()):.1e})")
        self.logger.info(f"[step {self.step}{accstep}]: loss={c_num:.5f}")
        self.logger.flush()

        if self.accum_step == self.args.batch_split:
...
def main(args):
    ...
    train_net = train_step(model, cri, optim, mixup, args, logger)
    train_net.set_train()
